...
data_formation = pd.read_excel('D:\\PI\\ProjtPI_Final\\dataaaa\\Monaco-all-result.xlsx')

# Charger les données de classement
data_rank = pd.read_excel('D:\\PI\\ProjtPI_Final\\dataaaa\\understat.com.xls')
club = data_rank['Squad'].unique()
# Charger les données des joueurs
data_players = pd.read_excel('D:\\PI\\ProjtPI_Final\\dataaaa\\joueursssss.xlsx')
joueurs_value= data_players['short_name'].unique()
# playerURL=data_players['player_url'].unique()

# Charger les données des Commentaire
data = pd.read_excel('D:\\PI\ProjtPI_Final\\dataaaa\\data.xlsx')

# Charger le modèle SVM depuis le fichier pickle
model = joblib.load("D:\\PI\\ProjtPI_Final\\dataaaa\\model_Bless.pkl")

# Charger les données nécessaires
data_bless = pd.read_excel("D:\\PI\\ProjtPI_Final\\dataaaa\\DimBlessure.xlsx")
...
def predict():
    ...
    if nom_joueur in features['Short Name'].values:
        ...
        if tendance_blessure and blessure_recente:
            raison = "Tendency to be injured often and recent injury"
        elif tendance_blessure:
            raison = "Tendency to be injured often"
        elif blessure_recente:
            raison = "Recent injury"
        else:
            raison = "No tendency to be injured often"
        pre=prediction[0]
        gravite_blessure = "Severe" if type_blessure_grave else "Nevermind"
...
def predict_player_value():
    ...
    if 'player_name' in req_data:
        ...
        if not player_data.empty:
            ...
            player_url = player_data['player_url'].iloc[0]
            return jsonify({
